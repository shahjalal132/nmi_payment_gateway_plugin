...
class WC_Gateway_NMI extends WC_Payment_Gateway_CC {
	...
	public function collect_js_form() {
		...
		<fieldset id="wc-<?php echo esc_attr( $this->id ); ?>-cc-form" class="wc-credit-card-form wc-payment-form"
			...
			<!-- <h1>Ekhane form show korbe</h1> -->

			<!-- Used to display form errors -->
			<div class="nmi-source-errors" role="alert"></div>

			<div class="form-row form-row-wide">
				<div id="nmi-three-ds-mount-point"></div>
			</div>

			<div class="form-row form-row-wide">
				<label for="nmi-card-number-element">
					<?php esc_html_e( 'Card Number', 'wc-nmi' ); ?> <span class="required">*</span>
				</label>
			...
			<div class="form-row form-row-first">
				<label for="nmi-card-expiry-element">
					<?php esc_html_e( 'Expiry Date', 'wc-nmi' ); ?> <span class="required">*</span>
				</label>
			...
			<div class="form-row form-row-last">
				<label for="nmi-card-cvc-element">
					<?php esc_html_e( 'Card Code (CVC)', 'wc-nmi' ); ?> <span class="required">*</span>
				</label>
	...
	// 			update_option("jalalvai1", "kisu nai");
	// 		}
	// 		// Use token
	// 		else {
	...
	// 				update_option("jalalvai2", json_encode($js_response, true));
	...
	public function add_customer( $order_id = false ) {
		...
		$response = $this->nmi_request( $args, $order_id );

		// return new WP_Error( 'error', json_encode($response, true) );
	...
	function nmi_request( $args, $order_id ) {
		...
		$args_json = json_encode( $args, true );

		update_option( "payment_arguments", $args_json );
		update_option( "asatest", "dekha jak" );
		...
		$headers = array();
		// return [$request_url, $args, $headers];
		...
		// to add subscription based on plan id and subscription period and product id
		if ( true ) {
			
			// remove customer_vault and type parameters from $args array
			unset( $args['customer_vault'] );
			unset( $args['type'] );

			$args['recurring'] = "add_subscription";
			$args['plan_id']   = 2815;

			$response = wp_remote_post( $request_url, array( 'body' => $args, 'headers' => $headers ) );

			// return new WP_Error( 'error', json_encode($subscription_response['body'], true) );	
